...
const count = (target, input) => {
  // input.map( (val, idx) => {
  //   if (val.number === target) return val.number;
};
...
const totalSum = (input) => {
  // // Define a totalSum variable.  
  // // for each array, reduce and sum the amounts.
  // // add the reduced sum to a totalSum variable.
  // let totalSum = 0;
  // for (let arr in input) {
  //     input.reduce( (accumulator, currentVal) => {
  //       totalSum = (accumulator + currentVal);
  //   })
  //   return totalSum;
};
